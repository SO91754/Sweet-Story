...
async function load(){
 ...
 <div class="actions">${o.hasReference?`<a class="btn secondary" href="/api/admin/orders/${encodeURIComponent(o.id)}/reference" target="_blank">Referentiefoto</a>`:""}<button class="btn primary" data-status="confirmed" data-id="${esc(o.id)}">Bevestigd</button><button class="btn secondary" data-status="rejected" data-id="${esc(o.id)}">Afwijzen</button><button class="btn secondary" data-status="done" data-id="${esc(o.id)}">Afgehaald</button><button class="btn secondary" data-delete="${esc(o.id)}" style="border-color:#d99;color:#a33">Verwijderen</button></div>
 </article>`).join("");
 list.querySelectorAll("button[data-status]").forEach(b=>b.addEventListener("click",()=>setStatus(b.dataset.id,b.dataset.status)));
 list.querySelectorAll("button[data-delete]").forEach(b=>b.addEventListener("click",()=>deleteOrder(b.dataset.delete)));
}
...
async function deleteOrder(id){
 ...
 try{await api(`/api/admin/orders/${encodeURIComponent(id)}`,{method:"DELETE"});await load()}catch(e){alert(e.message)}
}
